...
This is an example of something that is at the module level, `ORG_TRANSCRIPT_DIR` is constructed from a path defined in config.py and will therefore fail if that path has not been specified at the time of module import. The solution to this is to make sure that this code only executes when Chatino language is actually being processed, this will involve a refactor.
